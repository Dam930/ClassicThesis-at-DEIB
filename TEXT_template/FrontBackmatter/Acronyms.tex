    % environment acronym take the longest acronym as parameter to
    % determine the wideness of columns. Put it between square bracket
    \begin{acronym}[EMODP]
    	% put here the acronyms you use and recall them in text by
    	% using \ac{NAME}. In the text you can also use \acs{}, \acf{}, \acl{}
    	% for various flavours
    	\acro{TRPO}{Trust Region Policy Optimization}
	    \acro{KL}{Kullback-Leibler Divergence}	
		\acro{SVRPG}{Stochastic Variance Reduced Policy Gradient}
		\acro{SVRG}{Stochastic Variance Reduced Gradient}
    	\acro{MDP}{Markov Decision Processes}
         \acro{RL}{Reinforcement Learning}
        \acro{SL}{Supervised Learning}
        \acro{SG}{Stochastic Gradient}
        \acro{FG}{Full Gradient}
        \acro{SAG}{Stochastic Average Gradient}
        \acro{SAGA}{Stochastic Average Gradient}
        \acro{ADAM}{ADAptive Moment estimation}
        \acro{PGT}{Policy Gradient Theorem}
        \acro{MSPBE}{empirical mean squared projected Bellman error}
    \end{acronym}