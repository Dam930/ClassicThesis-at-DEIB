...
Figure \ref{fig:swimmerone} shows self-normalized \acs{SVRPG} against G(PO)MDP. Our algorithm outperforms G(PO)MDP for almost the entire learning process. Here, toward the end of the learning process, we note that the improvement becomes statistically significant.
...
\begin{align}\label{E:svrpg.c.version}
	\blacktriangledown J(\vtheta_{t}) &:= v_t= \omega(\tau_i|\wt{\vtheta}, \vtheta_t)\wh{\nabla}_N J(\wt{\vtheta})\\ \notag
	& \quad{} + 
		\frac{1}{B} \sum_{i=0}^{B-1} \left[
