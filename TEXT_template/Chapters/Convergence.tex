...
Each of the three challenges presented can potentially prevent convergence, so we need additional assumptions that we present in Section \ref{sec:assumption}. In Section \ref{sec:gaussianassumption} we show how Gaussian policies satisfy these assumptions. In Section \ref{sec:definition} we give some additional definitions which will be useful in the proofs. In Section \ref{sec:ancillarylemmas} we provide the ancillary lemmas for convergence proof. In Section \ref{sec:maintheorem} we provide the enunciated and the proof of the convergence theorem. Finally in Section \ref{sec:consequence} we provide the consequences of the convergence theorem.
...
\begin{definition}
	...
	\]
	and represent the difference between our estimate of gradient (\eg full gradient) and the real gradient.
\end{definition}
