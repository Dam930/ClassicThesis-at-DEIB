...
Since we cannot compute an exact full gradient due to the infinity of dataset, we require the variance of the estimator of gradient to be bounded.
...
Given this assumptions we can prove convergence guaranties of \acs{SVRPG}.

\section{Assumptions in gaussian policies}\label{sec:gaussianassumption}
We can now provide more details on the applicability of convergence guaranties on the case of Gaussian policies. Gaussian policies are the most used policies in practice. In our experiment we use Gaussian policies too. We start to analyze the case of one-dimensional bounded action space $\mathcal{A}\subset\mathbb{R}$, linear mean $\mu(s) = \vtheta^T\vphi(s)$ and fixed standard deviation $\sigma$:
\[
...
As mentioned, \citep{pirotta2013adaptive} provides a bound on the variance of the REINFORCE estimator, adapted from \cite{zhao2011analysis}, which does not depend on $\vtheta$:
\begin{align*}
...
So, also Assumption \ref{ass:M2} has been satisfied.


We now briefly examine some generalizations of the simple Gaussian policy defined above that can be found in applications, note that all this cases are used in our experiments and in particular are used almost always in real applications:
...
\begin{definition}
	...
	\end{align*}
	where the sequence $\wt{\vtheta}^s,\vtheta_0^{s+1},\dots,\vtheta_t^{s+1}$ is defined in Algorithm \ref{alg:svrpg}, $N = [0,\dots,N)$, and $B = [0,\dots,B)$. To avoid inconsistencies, we also define $\Ets[(-1)]{X} \coloneqq \Es{X}$.
...
\begin{proof}
	...
	&+ \int_{\Tspace}\pol(\tau)\Dij{}{\theta}\log p(\tau|\vtheta)\Reward(\tau)\de \tau \nonumber\\
...
\begin{proof}
	...
	&\qquad+ \Ets[t-1]{\dotprod{e_s}{\varphi(\vtheta_t^{s+1})}} \label{eq:6}\\
	&=
	\Ets{\dotprod{\gradJ{\vtheta_t^{s+1}}}{\varphi(\vtheta_t^{s+1})}} \nonumber\\
	&\qquad+ \Ets[t-1]{\dotprod{e_s}{\varphi(\vtheta_t^{s+1})}} \label{eq:7}\\
	&=
	\Ets{\dotprod{\gradJ{\vtheta_t^{s+1}}}{\varphi(\vtheta_t^{s+1})}} \nonumber\\
	&\qquad+
...
\begin{proof}
	...
	\end{align}
	where~\eqref{eq:11} is from the L-smoothness of $J(\vtheta)$ \cite{nesterov2013introductory} and \eqref{eq:12} is from the \acs{SVRPG} update.
	...
	&\qquad+\frac{\alpha_t}{2}\left(1-\frac{2c_{t+1}}{\beta_t}\right)\Ets[t-1]{\norm[]{\gradJ{\vtheta_t^{s+1}}}^2} \nonumber\\
